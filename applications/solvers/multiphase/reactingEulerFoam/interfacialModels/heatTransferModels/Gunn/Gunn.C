...
Foam::heatTransferModels::Gunn::K(const scalar residualAlpha) const
{
    ...
    volScalarField sqrAlpha2 = sqr(alpha2);

    volScalarField Nu
    (
        (scalar(7) - 10.0*alpha2 + 5.0*sqrAlpha2)
            *(scalar(1) + 0.7*pow(pair_.Re(), 0.2)*cbrt(pair_.Pr()))
      + (1.33 - 2.4*alpha2 + 1.2*sqrAlpha2)*pow(pair_.Re(), 0.7)
            *cbrt(pair_.Pr())
    );
