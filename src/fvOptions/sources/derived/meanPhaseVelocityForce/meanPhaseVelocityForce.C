...
:
    ...
    alphaName_(coeffs_.lookup("alphaName")),
    alpha_(mesh.lookupObject<volScalarField>(alphaName_)),
    rhoName_(coeffs_.lookup("rhoName")),
...
Foam::scalar Foam::fv::meanPhaseVelocityForce::alphaRhoMagUbarAve
(
    const volVectorField& U
) const
{

    scalar alphaRhoMagUbarAve = 0.0;
    ...
    forAll(cells_, i)
    {
        ...
        alphaRhoMagUbarAve +=
            alpha_[celli]*rho_[celli]*(flowDir_ & U[celli])*volCell;
    }

    reduce(alphaRhoMagUbarAve, sumOp<scalar>());
    alphaRhoMagUbarAve /= V_;

    return alphaRhoMagUbarAve;
}
...
void Foam::fv::meanPhaseVelocityForce::correct(volVectorField& U)
{
    ...
    forAll(cells_, i)
    {
        label celli = cells_[i];
        scalar volCell = cv[celli];
        alphaRhoAvg += alpha_[celli]*rho_[celli]*volCell;
        rAUavg += rAU[celli]*volCell;
    }

    // Collect across all processors
    reduce(alphaRhoAvg, sumOp<scalar>());
    reduce(rAUavg, sumOp<scalar>());

    // Volume averages
    alphaRhoAvg /= V_;
    rAUavg /= V_;

    scalar alphaRhoMagUbarAve = this->alphaRhoMagUbarAve(U);

    // Calculate the pressure gradient increment needed to adjust the average
    // flow-rate to the desired value
    dGradP_ = relaxation_*(alphaRhoAvg*mag(Ubar_) - alphaRhoMagUbarAve)/rAUavg;
    ...
    Info<< "Pressure gradient source: uncorrected Ubar = "
        << alphaRhoMagUbarAve/alphaRhoAvg
